...
function! <SID>GetDoxygenStyleDoc(fun_info, leading_blank)
	...
	if (strlen(ret_type)>0) 
		...
		let doc = doc . a:leading_blank." * @retval  0 means sucess"." \n"
	endif

	"other
	"let doc = doc . a:leading_blank." * @see \n"
	"let doc = doc . a:leading_blank." * @note \n"
	let doc = doc . a:leading_blank." * @author luyao\n" 
...
function! <SID>GetDoxFileHeader(leading_blank)
	...
	let doc = doc. a:leading_blank."/***************************************************************************\n"
	let doc = doc. a:leading_blank." * \n"
	let doc = doc. a:leading_blank." * Copyright (c) ".<SID>GetYear()." izptec.com, Inc. All Rights Reserved\n"
	...
	let doc = doc. a:leading_blank." * @author Yao Lu(yaolu1103@gmail.com)\n"
	let doc = doc. a:leading_blank." * @date ".<SID>GetDate()."\n"
	let doc = doc. a:leading_blank." * @version 1.0 \n"
	let doc = doc. a:leading_blank." * @brief \n"
	let doc = doc. a:leading_blank." *  \n"
	let doc = doc. a:leading_blank." **/\n"
	call append(line('$'), "/* vim: set ts=256 sw=4 sts=4 tw=80 expandtab noet: */")
	return doc


endfunction

function! <SID>GetDoxShellHeader(leading_blank)
	...
	let doc = doc. a:leading_blank."\#/**\n"
	let doc = doc. a:leading_blank."\# * @file ".<SID>GetFileName()."\n"
	let doc = doc. a:leading_blank."\# * @author YAO LU(luyao@izptec.com)\n"
	let doc = doc. a:leading_blank."\# * @date ".<SID>GetDate()."\n"
	let doc = doc. a:leading_blank."\# * @version 1.0 \n"
	let doc = doc. a:leading_blank."\# * @brief \n"
	let doc = doc. a:leading_blank."\# *  \n"
...
function! <SID>GetDoxFH(type)
	...
	let cur_line = line(".")
	let first_line = getline(cur_line)
	let leading_blank = matchstr(first_line,".", 0)
	if (a:type == 1) 
...
function! <SID>note()
	...
	let note = "/* @NOTE:".<SID>GetUserName()
	let note = note." #Date:".<SID>GetDate()
    call append(line('.'), " */")
    call append(line('.'), " *")
...
function! <SID>InsertPerlHead()
	...
	while (idx > 0)
		...
		let idx = idx - 1
	endwhile
	call append(line('$')-1,"=cut")
	call cursor(17, 12)

endfunction

function! <SID>InsertShellHead(type)
    ...
    elseif (a:type == "go")
        call append(0,"/*in/ls >/dev/null")
    	call append(line('$') - 1, "gccgo -c $0")
	    call append(line('$') - 1, "NAME=`echo $0 | awk -F '/' '{print $NF}' | sed 's/\.go//g'`")
	    call append(line('$') - 1, "gccgo -o $NAME $NAME.o")
	    call append(line('$') - 1, "exit")
    endif

	call append(line('$') - 1, "\# Program:")
	call append(line('$') - 1, "\# \t\t")
	call append(line('$') - 1, "\# History:")
	call append(line('$') - 1, "\# Author: luyao(yaolu1103@gmail.com)")
    call append(line('$') -1 , "\# Date:  ".<SID>GetDate())
	call append(line('$') - 1, "")

    if (a:type == "sh")
	    call append(line('$') - 1, "PATH=/bin:/sbin:/usr/bin:~/bin:/usr/local/bin:/usr/local/sbin")
        call append(line('$') - 1, "export PATH")
    elseif (a:type == "perl")
        call append(line('$') - 1, "use strict;")
    elseif (a:type == "go" )
        call append(line('$')-1, "*/")
    endif
...
"map <leader>a :call <SID>GenDoc()<CR>
"map <F3> :call <SID>ValComment()<CR>
autocmd FileType c,cpp,java,sh,awk,vim,sed,perl,php,go nnoremap <silent> <Leader>a :call <SID>GenDoc()<CR>
autocmd FileType c,cpp,java,sh,awk,vim,sed,perl,php,go nnoremap <silent> <Leader>2 :call <SID>note()<CR>
autocmd FileType c,cpp,java,sh,awk,vim,sed,perl,php,go nnoremap <silent> <Leader>s :call <SID>ValComment()<CR>
autocmd FileType c,cpp,java,sh,awk,vim,sed,php,go	  nnoremap <silent> <Leader>1 :call <SID>ShortComment()<CR>
" autocmd FileType c,cpp,java,sh,awk,vim,sed,perl,python,php,go nnoremap <silent> <Leader>2 :call <SID>GetDoxFH(2)<CR>
" autocmd FileType c,cpp,java,sh,awk,vim,sed,perl,python,php,go nnoremap <silent> <Leader>3 :call <SID>GetDoxFH(3)<CR>
autocmd BufNewFile *.h,*.cpp,*.c,*.cc,*.java :call <SID>InsertFormat()
autocmd BufNewFile *.php :call <SID>InsertPHPHead()
autocmd BufNewFile *.pl,*.PL :call <SID>InsertShellHead("perl")
autocmd BufNewFile *.sh :call <SID>InsertShellHead("sh")
autocmd BufNewFile *.go :call <SID>InsertShellHead("go")
autocmd BufNewFile *.py :call <SID>InsertShellHead("python")
